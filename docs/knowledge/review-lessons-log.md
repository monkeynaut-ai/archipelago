...
### Schema extensions as round-trip metadata channels
**Principle:** Coherence, Clarity, Surface/verify/encode

**Description:** JSON Schema reserves `x-*` keys for vendor/tool extensions, and every well-behaved validator ignores keys it doesn't recognize. That lets you attach metadata to a field that describes a **host-side capability contract** — "this is a file path I will verify," "this field holds a secret the platform will redact," "this field identifies a cost center" — and send the schema through a third-party interface (an LLM's structured-output mode, an HTTP API, a code generator) that has no opinion about your extension. The extension travels along for the ride, gets picked up on the return trip, and drives whatever post-processing your system needs. Combined with a Pydantic `Annotated[..., Marker()]` declaration, the metadata lives next to the field it describes — one source of truth, navigable via LSP, surviving schema transformations as long as the flattener preserves unknown keys.

**How to apply:** When you find yourself maintaining a parallel list ("these fields are secrets," "these fields contain paths," "these fields should be redacted") alongside the data model, try replacing it with an `Annotated[FieldType, Marker(config=...)]` whose `__get_pydantic_json_schema__` emits `{"x-your-domain-marker": {...}}` onto the field's schema. Then read the extension back in the consumer (e.g., walk the schema for `x-your-marker` keys). Verify the extension survives your schema-flattening / compatibility-shimming path; once through, every round-trip of that schema carries the marker without effort. Prefer `x-<product>-<purpose>` naming so multiple extensions don't collide.

**Benefit:** One declaration replaces two parallel representations (field + marker list) that could drift. Third-party consumers (LLMs, codegen tools, schema validators) continue to work — they ignore the extension — so there's no conformance fight. The marker's presence is locally visible where the field is declared, which is where a future maintainer will look when they ask "what special thing happens to this field?" Extensions compose: `Annotated[str, AgentFilePath(), Sensitive()]` is unambiguous and extensible without reshaping the data model.
